...
### $form.openPage( string pagePath, string route );
...
### $vm.getFieldData( string fieldPath );

1. 예시 : var name = $vm.getFieldData("hoga.responsePack.infoBlock.name") + " 짜잔"; <br/>
설명 : 해당 필드의 데이터를 읽어온다. DS 이름은 "hoga.responsePack.infoBlock.name" 이다. <br/>
CDS로 읽어온 데이터중 name필드에 “+짜잔" 이라는 텍스트를 추가한다음 name 이라는 변수에 적용한다. <br/>
아래처럼 미리설정한 CDS 에서 필드는 아래와 같다. <br/>

2. 예시 : let cherate = $vm.getFieldData("E1413_ds.responsePack.OutBlock.cherate"); <br/>
설명 : DS 필드에서 데이터를 읽어서 변수에 넣는다. 


### $vm.updateFieldStringData( string fieldPath, string data );

1. 예시 : $vm.updateFieldStringData("hoga.callPack.code", "035420");  <br/>
설명 : 필드에 문자열 데이터를 업데이트 한다. 여기서 필드란 컨텐츠 데이터 소스(CDS)이며 위 처럼 설정한다. <br/>


### $vm.updateFieldIntData( string fieldPath, int data );
1. 예시 : $vm.updateFieldIntData( "hoga.callPack.code", data ); <br/>
설명 : 필드에 숫자형(int) 데이터를 업데이트 한다.



### $vm.deleteRecordAt( string recordPath, int count );
...
ProvInstance 서비스를 요청하기전에 무언가를 설정하기 위한  함수. 
보통 Key 값을 셋팅하기 전에 사용될거라 예상됨.

예시 
function onRequestBefore(provInstName) {
if (provInstName == "E1611_0")  {
    let gubun = $vm.getFieldData("chart_ds.callPack.inBlock0.gubun"); 
    //logger.e(" E1611_0 의 INPUT 값입니다. : " + gubun);
  }
}
...
### onResponseUpdateAfter (contentsDSName);
...
실시간
onPushUpdateBefore(contentsDSName);


onPushUpdateAfter(contentsDSName);
실시간 데이터 받은뒤에 이벤트를 처리한다. CDS에 데이터를 올린뒤에 이벤트를 처리한다.


사용예시 

function onPushUpdateAfter (contentsDSName) {
  if(contentsDSName == "E1110_ds") {
...
logger(); 
루시 캔버스 창에는 하단에 Console, Packet 창이 있다. 이중 Packet 창에 로그를 남긴다. 

사용예시
logger.i(“Info  로그입니다.”);
logger.e(“Error 로그입니다. 현재시간 : “ + Date() );
logger.d(“Debug 로그입니다.arg code: “ + arg.code ); 
